...
class AlipayForm(AlipayCommon):
    ...
    def test_10_alipay_form_render(self):
        ...
        self.alipay.write({'alipay_email_account': 'tde+alipay-facilitator@odoo.com', 'fees_active': False})
        ...
        form_values = {
            'cmd': '_xclick',
            'business': 'tde+alipay-facilitator@odoo.com',
        ...
        self.assertEqual(tree.get('action'), 'https://www.sandbox.alipay.com/cgi-bin/webscr', 'alipay: wrong form POST url')
    ...
    def test_11_alipay_form_with_fees(self):
        ...
        for form_input in tree.input:
            if form_input.get('name') in ['handling']:
                handling_found = True
                self.assertEqual(form_input.get('value'), '1.57', 'alipay: wrong computed fees')
        self.assertTrue(handling_found, 'alipay: fees_active did not add handling input in rendered form')
    ...
    def test_20_alipay_form_management(self):
        ...
        # check
        self.assertEqual(tx.state, 'pending', 'alipay: wrong state after receiving a valid pending notification')
        self.assertEqual(tx.state_message, 'multi_currency', 'alipay: wrong state message after receiving a valid pending notification')
        self.assertEqual(tx.acquirer_reference, '08D73520KX778924N', 'alipay: wrong txn_id after receiving a valid pending notification')
        self.assertFalse(tx.date_validate, 'alipay: validation date should not be updated whenr receiving pending notification')
        ...
        alipay_post_data['payment_status'] = 'Completed'
        # validate it
        tx.form_feedback(alipay_post_data, 'alipay')
        # check
        self.assertEqual(tx.state, 'done', 'alipay: wrong state after receiving a valid pending notification')
        self.assertEqual(tx.acquirer_reference, '08D73520KX778924N', 'alipay: wrong txn_id after receiving a valid pending notification')
        self.assertEqual(tx.date_validate, '2013-11-18 03:21:19', 'alipay: wrong validation date')
